...
const MatchSchedulerInput = ({ matchDetailsRef }: MatchSchedulerInputProps) => {
  const [selected, setSelected] = useState<Date | null>(null);
  const [selectedHour, setSelectedHour] = useState<{ id: number; time: string }>({
    id: 1,
    time: '00:00'
  });
    ...
    <Div px={customTheme.spacing.medium}>
      ...
      <Div mt={customTheme.spacing.medium} style={{ gap: verticalScale(8) }}>
        {schedules.map((schedule, index) => (
          <TouchableOpacity key={index} onPress={() => setSelectedHour(schedule)}>
            <Div
              h={verticalScale(48)}
              bg={
